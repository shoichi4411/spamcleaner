...
//			mb_check_encoding ([ string $var [, string $encoding ]] )
		...
		if( !empty( $header['content-id'] ) ){
				$ret['content_id'] = $header['content-id'];
		}
...
/**
 * QdImap ver 0.1.0
 * Imap Receiver for PHP
 *
 * PHP versions 4 and 5 (PHP4.3 upper)
 *
 * Licensed under The MIT License License
 *
 * @copyright		Copyright 2013, Shoichi Takahashi@R-ONE Computer.
 * @link			http://f-dev.jp
 * @version			0.1.0
 * @lastmodified	2013-01-10
 * @license			The MIT License http://www.opensource.org/licenses/mit-license.php
 */

class QdImapBase extends QdDecodeBase{
	...
	//--------------------------
	function __construct( $param = null ){
		$this->QdImapBase($param);
	}
	function QdImapBase( $server ){
		if( isset( $server[0] ) ){
			$this->server = array_merge( $this->server , $server[0] );
		}
		parent::__construct( $server );
	...
	function connect(){
		...
		stream_set_timeout ( $this->fp , $this->time_out );
		list($fg1,$void)=$this->communicate("CAPABILITY",array('Error',__LINE__,!$this->error_fatal_ignore),true);
		list($fg2,$void)=$this->communicate("LOGIN ".$this->server['user']." ".$this->server['pass'],array('USER ID or PASS ID Error',__LINE__,!$this->error_fatal_ignore),true);
		$this->uid_list	=array();
		return  $fg1 && $fg2;
	}
	function close(){
		$_ret = $this->cmd('LOGOUT',null,true);
		return fclose($this->fp) && $_ret;
	}
	function done(){
		return $this->close();
	...
	function selectMailbox($mailbox) {
		$this->preCheck();
		$ret = $this->cmd('SELECT '.$mailbox,null,true);
		foreach($ret as $line) {
			if (preg_match("/^\* ([0-9]+?) EXIST/", $line, $matches)) {
				$this->count = (INT)$matches[1];
				break;
			}
		}
	}
	function count( $retry = false ){
		...
		$this->preCheck();
		$ret = $this->cmd('UID SEARCH ALL',null,true);
		foreach($ret as $line) {
			if (preg_match("/^\* SEARCH /", $line, $matches)) {
				$ids = explode(" ", $line);
				$this->count = count($ids) - 2;
				break;
			}
		}
		return $this->count;
	...
	// OverRide Parent by POP
	function getMail($uid = null){

		$this->_before_getMail();

		if( null === $uid ){
			$all = $this->getTop( null );
		}else{
			$this->preCheck();
			( 0 !== $this->count ) or $this->count();
			if( ( 0 == $this->count) ){
			...
			$mail = $this->cmd('UID FETCH '.$uid.' (BODY.PEEK[] UID)', null , false , false );
			!$this->delete or $this->delete($i);
			$all = $mail[1];
		}
	...
	function getTop( $msg_num = null ){
		$this->preCheck();
		( 0 !== $this->count ) or $this->count();
		if( ( 0 == $this->count)  ){
			return false;
		}
		$msg_num = is_null( $msg_num ) ? $this->uid : $msg_num;
		$ret = $this->cmd('FETCH 1:* UID', null, false, true);
		preg_match("/^\* [0-9]+? FETCH \(UID ([0-9]+?)\)/", $ret[count($ret) - 2], $matches);
		$this->uid = $matches[1];
		$mail = $this->cmd('UID FETCH '.$this->uid.' (BODY.PEEK[] UID)' , null , false , false );
		return $mail[1];
	}
	...
	function getUid( $msg_num = null ){
		...
		$msg_num = is_null( $msg_num ) ? '1:*' : $msg_num;
		$ret = $this->cmd('FETCH '.$msg_num.' UID', null, false, true);
		preg_match("/^\* [0-9]+? FETCH \(UID ([0-9]+?)\)/", $ret[count($ret) - 2], $matches);
		$this->uid = $matches[1];
	...
	function getUidAll(){
		...
		$this->uid_list = array();
		foreach($ret as $line){
			if (preg_match("/^\* ([0-9]+?) FETCH \(UID ([0-9]+?)\)/", $line, $matches)) {
				$this->uid_list[$matches[1]] = $matches[2];
			}
		}
	...
	function delete( $num , $done = false ){
		if(!is_numeric($num)){
			$this->error( 'Specifed Error , delete command needs numeric' , __LINE__ );
		}

		if ($this->uid == null) {
			$this->uid = $this->getUid($num);
		}

		$ret1 = !( false === $this->cmd('UID STORE '.$this->uid.' +FLAGS (\Deleted)', null , false, true ) );
		$ret2 = true;
		if( $done ){
			$ret2 = $this->done();
		}
	...
	function deleteUid( $uid ){
		...
		foreach($uid as $id) {
			$ret = $this->cmd('UID STORE '.$id.' +FLAGS (\Deleted)', null , false, true );
		}
		return $ret;
	}

	function expunge() {
		$ret = $this->cmd('EXPUNGE', null , false, true );
		return $ret;
	}


	//--------------------------------------------
	// for Communication to Imap Server
	//--------------------------------------------
	function cmd( $cmd , $param=null , $line_1=false , $array = true ){
		$cmd .= isset($param) ? ' '.$param:null;
		list( $fg , $ret ) = $this->communicate( $cmd , array('Error Command '.$cmd,__LINE__,true) , $line_1,$array);
	...
	function getTag() {
		$this->tag = $this->tag + 1;
		$this->_lastTag = sprintf("A%04d", $this->tag);
	...
	function communicate( $put_message , $err = null , $line_1 = false , $array=true){
		...
		$this->debugEchoLine('Client: ',$put_message);
		fputs( $this->fp , $this->getTag()." ".$put_message."\r\n");
		$ret = $this->getMessage($array);
		if( 'OK' === strtoupper( substr( $ret[count($ret) - 1] , 6 , 2 ) )){
			$fg = true;
		}else{
			$er = print_r(stream_get_meta_data ( $this->fp ) , true );
			$this->error( 'Comunicate Error, stream_get_meta_data=>'.$er ,__LINE__);
	...
	function getMessage($array = true){
		if(!$this->preCheck()){
			return false;
		}
		...
		do {
			...
			$this->debugEchoLine('Server: ',$r);
			$ret[] = $r;
		} while($this->_lastTag != substr($r, 0, 5));

		if ($array) {
			return $ret;
		}else{
			$newRet[0] = $ret[0];
			$newRet[2] = $ret[count($ret) -1];
			array_shift($ret);
			array_pop($ret);
			$newRet[1] = implode('', $ret);
			return $newRet;
		}
	}
}
...
class QdImap extends QdImapBase{
	var $name			= 'QdImap';

	function QdImap( $param = null ){
		parent::__construct( $param );
	}
...
if(!function_exists('qd_receive_mail')){
	function qd_receive_mail( $cmd , $param = null , $charset = null){
		...
		if( 'imap' == $cmd ){
			$receive[0] = QdmailReceiver::start( 'imap' , $param , $charset);
		}
		if( 'stdin' == $cmd ){
